...
export const ContributeModal = ({ group, onClose, onContributed }: ContributeModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!memberName.trim() || isNaN(parsedAmount) || parsedAmount <= 0) {
      setError('Member name and a positive amount are required.');
      return;
    }
    setLoading(true);
    setError(null);
    try {
      await addContribution(group.id, { memberName, amount: parsedAmount });
      onContributed();
    } catch {
